<template>
  <div class="break-words z-50 fixed top-0 left-0 w-full flex items-center justify-center bg-black/80 px-3 sm:px-0 h-full">
    <div class="bg-black break-words w-11/12 md:w-1/3 lex flex-col justify-center items-center relative border-golden 0 border-2">
      ...
      <div class="break-words px-6 sm:px-10 w-full">
        <div class="break-words w-full flex flex-col justify-center items-center mt-2 mb-10">
          <div v-if="loading" class="break-words w-full text-white text-xl pb-3 font-bold text-center sm:text-center mt-2 mb-2">
            Excluindo, por favor aguarde...
          </div>
          <div v-else class="break-words w-full text-white text-xl pb-3 font-bold text-center sm:text-center mt-2 mb-2">
...
<script>
import { getDragonById, deleteDragon } from '@/firebase/dragons';
...
export default {
  ...
  async created() {
    const dragon = await getDragonById(this.id);
